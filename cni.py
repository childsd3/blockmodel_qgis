...
def main():
	blockModelFile = 'blkmdl.dat'
	ascSurfaceFile = 'surface.asc'
	print('Loading ASC surface.')
	surfaceData,blankASC,sncols,snrows,sxllcorner,syllcorner,scellsize,sNODATA_value = loadASC(ascSurfaceFile)
	print('ASC surface loaded.')
	print('Finding block model limits.')
	bminX,bminY,bminZ,bnCols,bnRows,bnLvls,bcolSize,browSize,blvlSize = blockModelLimits(blockModelFile)
	print('Initializing 3d block model.')
	blockModel = make3dList(bnCols,bnRows,bnLvls)
	print('Block model initialized.')
	...
	with open(blockModelFile, 'r') as f:
		...
		for line in f:
			...
			if counter == 1:
				continue
			lineArray = line.split(",")
			xValue = float(lineArray[0])
			yValue = float(lineArray[1])
			zValue = float(lineArray[2])
			icol,irow,ilvl = xyz2index(xValue,yValue,zValue,bminX,bminY,bminZ,bcolSize,browSize,blvlSize)
			try:
				blockModel[icol][irow][ilvl] = float(lineArray[3])
			except:
				print(icol,irow,ilvl,float(lineArray[3]))
	print('Building ASC')
	for i in range(len(blankASC)):
		for j in range(len(blankASC[0])):
			x,y = index2xy(i,j,sxllcorner,syllcorner,scellsize)
			value = surfaceData[i][j]
			try:
				bi_row,bi_col,bi_lvl = xyz2index(x,y,value,bminX,bminY,bminZ,bcolSize,browSize,blvlSize)
				blockValue = blockModel[bi_row][bi_col][bi_lvl]
				blankASC[i][j] = blockValue
				if x < 200 and x > -200 and y < 200 and y > -200:
					print(x,y,value,xyz2index(x,y,value,bminX,bminY,bminZ,bcolSize,browSize,blvlSize),blockValue)
			except:
				if x < 200 and x > -200 and y < 200 and y > -200:
					print(x,y,value)
				blankASC[i][j] = sNODATA_value
	script = generateASC(blankASC,sncols,snrows,sxllcorner,syllcorner,scellsize,sNODATA_value)
	outputFile = open('output.asc', 'w')
	outputFile.write(script)
	outputFile.close()
#		print(sorted(list(set(xArray)))[1]-sorted(list(set(xArray)))[0])
